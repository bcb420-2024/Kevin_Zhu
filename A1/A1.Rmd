...
# group by entrezgene_id and keep one row for each unique combination
unique_hgnc_symbols <- hgnc_symbols %>%
  group_by(entrezgene_id) %>%
  arrange(hgnc_symbol) %>%
  filter(!hgnc_symbol %in% lag(hgnc_symbol)) %>%
  slice(1) %>%
  ungroup()

# Merge the table with HGNC symbols
hgnc_tbl <- merge(filtered_tbl, unique_hgnc_symbols, 
                  by.x = "row.names", by.y = "entrezgene_id")

missing_values <- rownames(hgnc_tbl[hgnc_tbl$hgnc_symbol == "", ])
...
that do not encode functional proteins. 

```{r eval=TRUE}
# remove rows that do not have HUGO symbol
hgnc_tbl <- hgnc_tbl[!(hgnc_tbl$hgnc_symbol == ""), ]

# Set rownames using HGNC symbols and remove extra columns
rownames(hgnc_tbl) <- hgnc_tbl$external_gene_name
